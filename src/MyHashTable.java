...
public class MyHashTable {
	...
	public boolean addEmployee(EmployeeInfo theEmployee) {
        ...
        public EmployeeInfo removeEmployee(int intEmployeeNumber) {
                // Remove an employee from the hash table and return the reference value for that employee.
                int intBucketNumber = calcBucket(intEmployeeNumber);
                int intNumInArray = buckets[intBucketNumber].size();
                for (int intCounter = 0; intCounter < intNumInArray; intCounter++) {
                        EmployeeInfo someEmployee = buckets[intBucketNumber].get(intCounter);
                        if (someEmployee.getEmpNum()==intEmployeeNumber) {
                                buckets[intBucketNumber].remove(intCounter);
                                numInHashTable--;
                                return(someEmployee);
                        }
                }
        ...
        public EmployeeInfo findEmployee(int intEmployeeNumber) {
                // Remove an employee from the hash table and return the reference value for that employee.
                int intBucketNumber = calcBucket(intEmployeeNumber);
                int intNumInArray = buckets[intBucketNumber].size();
                for (int intCounter = 0; intCounter < intNumInArray; intCounter++) {
